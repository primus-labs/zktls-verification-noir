/**
 * Test 1: verify_comm — BusinessProgram (MAX_COMMS=65, MAX_MSGS_LEN=2054)
 *
 * Profiles and executes the large commitment-based verification on devnet.
 * Note: the 3.9M gate circuit exceeds the ClientIVC kernel floor (2^21) and
 * fails on proof generation.
 */
...
console.log("TEST 1: verify_comm — BusinessProgram (MAX_COMMS=65)");
console.log("=".repeat(70));
...
const result = await contract.methods.verify_comm(
  ...
  parsed.requestUrls, parsed.allowedUrls, parsed.commitments, parsed.randomScalars,
  parsed.msgsChunks, parsed.msgs, H, parsed.id,
).send({ from: account.address, fee: { paymentMethod }, wait: { timeout: 300000 } });
