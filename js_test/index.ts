...
const sigRS = sigBytes.subarray(0, 64);
const v = normalizeV(sigBytes[64]);
const signature = Array.from(sigRS);

// hash of packed data
const msgHash = Uint8Array.from(keccak_256(new Uint8Array(packedArr)));
const hash = Array.from(msgHash);

// recover pubkey
const pubkey = secp.recoverPublicKey(msgHash, Uint8Array.from(sigRS), v, false);
